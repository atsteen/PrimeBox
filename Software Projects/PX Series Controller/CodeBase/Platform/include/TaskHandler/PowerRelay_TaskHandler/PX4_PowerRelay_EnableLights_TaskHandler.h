...
class PX4_PowerRelay_EnableLights_TaskHandler
	: public PX4_PowerRelay_TaskHandler, public Logger
{
public:
	PX4_PowerRelay_EnableLights_TaskHandler()
...
inline bool PX4_PowerRelay_EnableLights_TaskHandler::HandleTask(TaskItem * ti)
{
	...
	if (_CanHandleTask(ti))
	{
		...
		_moduleGroup.LightRelayA()->Enable();
		_moduleGroup.LightRelayB()->Enable();

		#if defined(ENABLE_VERBOSE_LOGGING)
			char callStackStr[T_STACK_MAX_SIZE * 3];
			this->ComposeCallStack(callStackStr);
			this->DataLogger()->LogMessageVerbose(callStackStr, Viewstate_String_Generator::GetViewstateString(EVENT_TEXT_POWERRELAY_LIGHT_ON));
		#endif

		this->DataLogger()->LogSystemEventImmediate(EVENT_TEXT_POWERRELAY_LIGHT_ON);
...
inline bool PX4_PowerRelay_EnableLights_TaskHandler::_CanHandleTask(TaskItem * ti)
{
