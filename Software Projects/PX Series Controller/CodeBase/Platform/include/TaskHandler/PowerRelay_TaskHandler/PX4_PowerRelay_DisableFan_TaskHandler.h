...
class PX4_PowerRelay_DisableFan_TaskHandler
	: public PX4_PowerRelay_TaskHandler, public Logger
{
public:
	PX4_PowerRelay_DisableFan_TaskHandler()
...
inline bool PX4_PowerRelay_DisableFan_TaskHandler::HandleTask(TaskItem * ti)
{
	...
	if (_CanHandleTask(ti))
	{
		...
		_moduleGroup.FanRelay()->Disable();		

		#if defined(ENABLE_VERBOSE_LOGGING)
			char callStackStr[T_STACK_MAX_SIZE * 3];
			this->ComposeCallStack(callStackStr);
			DataLogger()->LogMessageVerbose(callStackStr, Viewstate_String_Generator::GetViewstateString(EVENT_TEXT_POWERRELAY_FAN_OFF));
		#endif

		DataLogger()->LogSystemEventImmediate(EVENT_TEXT_POWERRELAY_FAN_OFF);
...
inline bool PX4_PowerRelay_DisableFan_TaskHandler::_CanHandleTask(TaskItem * ti)
{
