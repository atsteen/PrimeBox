...
class PX4_InputPrompt_AlarmThresholdOverHumid_Select_TaskHandler
	: public TaskHandler
{
public:
	PX4_InputPrompt_AlarmThresholdOverHumid_Select_TaskHandler()
		: TaskHandler(
...
inline bool PX4_InputPrompt_AlarmThresholdOverHumid_Select_TaskHandler::_CanHandleTask(TaskItem * ti)
{
...
inline bool PX4_InputPrompt_AlarmThresholdOverHumid_Select_TaskHandler::HandleTask(TaskItem * ti)
{
	if (_CanHandleTask(ti))
	{
		switch (DataModel_PX4::GetInstance().GetSelectedViewstateElement()) {
		case SELECTABLE_BACK_ELEMENT:
			DataModel_PX4::GetInstance().SetSelectedViewstateElement(SELECTABLE_ENVIRONMENT_OVERHUM_ALARM_SET_ELEMENT, true);
