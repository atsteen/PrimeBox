...
class PX4_EnvironmentSettings_AlarmThresholdView_Select_TaskHandler
	: public TaskHandler
{
public:
	PX4_EnvironmentSettings_AlarmThresholdView_Select_TaskHandler()
		: TaskHandler(
...
inline bool PX4_EnvironmentSettings_AlarmThresholdView_Select_TaskHandler::_CanHandleTask(TaskItem * ti)
{
...
inline bool PX4_EnvironmentSettings_AlarmThresholdView_Select_TaskHandler::HandleTask(TaskItem * ti)
{
	if (_CanHandleTask(ti))
	{
		switch (DataModel_PX4::GetInstance().GetSelectedViewstateElement()) {
		case SELECTABLE_BACK_ELEMENT:
			DataModel_PX4::GetInstance().SetSelectedViewstateElement(SELECTABLE_MENU_SELECT_ENVIRONMENT_ALARM_THRESHOLDS_ELEMENT, true);			
		...
		case SELECTABLE_ENVIRONMENT_OVERTEMP_ALARM_SET_ELEMENT:
			DataModel_PX4::GetInstance().SetActiveViewstate(VIEWSTATEALIAS_INPUT_PROMPT_THREESEGMENT_ALARM_OVERTEMP);
			break;

		case SELECTABLE_ENVIRONMENT_OVERHUM_ALARM_SET_ELEMENT:
			DataModel_PX4::GetInstance().SetActiveViewstate(VIEWSTATEALIAS_INPUT_PROMPT_THREESEGMENT_ALARM_OVERHUMID);
			break;

		case SELECTABLE_ENVIRONMENT_LOGFULL_ALARM_SET_ELEMENT:
			DataModel_PX4::GetInstance().SetLogAlarmDefaultState(!DataModel_PX4::GetInstance().GetLogAlarmDefaultState());
			break;
