...
class PX4_ScheduledTasksConfig_EditTaskView_Select_TaskHandler
	: public TaskHandler, public Ipersistent
{
public:
	PX4_ScheduledTasksConfig_EditTaskView_Select_TaskHandler()
		: TaskHandler(
			TaskAlias::TASKALIAS_NAVIGATION_MENU_SELECT,
			ViewstateAlias::VIEWSTATEALIAS_SCHEDULED_TASK_EDIT_EXISTING_VIEW,
...
inline bool PX4_ScheduledTasksConfig_EditTaskView_Select_TaskHandler::_CanHandleTask(TaskItem * ti)
{
...
inline bool PX4_ScheduledTasksConfig_EditTaskView_Select_TaskHandler::HandleTask(TaskItem * ti)
{
	if (_CanHandleTask(ti))
	{
		...
		case SELECTABLE_BACK_ELEMENT:
			DataModel_PX4::GetInstance().SetSelectedViewstateElement(SELECTABLE_MENU_SELECT_SCHD_TASK_EDIT_EXISTING_ELEMENT, true);
			DataModel_PX4::GetInstance().SetActiveViewstate(VIEWSTATEALIAS_SCHEDULED_TASK_CONFIGURATION_VIEW);
			UpdatePersistentData(); //todo.. find a better way to do this... will not set edit persistent if timeout to status menu while in modal set view
			this->GetBuzzer()->playBackSelectTone();
			return true;

		case SELECTABLE_SCHEDULED_TASKLIST_EDIT_ELEMENT:
			DataModel_PX4::GetInstance().SetActiveViewstate(VIEWSTATEALIAS_INPUT_PROMPT_EDIT_TASK_TIME);
			break;
...
inline void PX4_ScheduledTasksConfig_EditTaskView_Select_TaskHandler::UpdatePersistentData()
{
