...
Viewstate_Element_Text_ScheduledTaskList_Edit::Viewstate_Element_Text_ScheduledTaskList_Edit(const int start_index, const int imbricationLevel, IModelTaskData * taskData, IModelViewstateData * viewstateData)
	: Viewstate_Element_Text(start_index, imbricationLevel, SELECTABLE_SCHEDULED_TASKLIST_EDIT_ELEMENT), _taskData(taskData), _viewstateData(viewstateData){}

Viewstate_Element_Text_ScheduledTaskList_Edit::~Viewstate_Element_Text_ScheduledTaskList_Edit(){}

int Viewstate_Element_Text_ScheduledTaskList_Edit::emplace(char * target, const int targetlen)
{
	...
	if (selectedListIndex < 3)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			{
				Viewstate_Element_Text_ScheduledTaskList_Edit_Row listRowVE_ScheduledTask(_startIndex + i*21, 1, taskDetail);
	...
	else
	{
		...
		for (int i = (((selectedListIndex + 1)/4) * 4) -1; i < (((selectedListIndex + 1) / 4) * 4) + 3; ++i) //todo... yikes that is fugly
		{
			...
			{
				Viewstate_Element_Text_ScheduledTaskList_Edit_Row listRowVE_ScheduledTask(_startIndex + (((i+1)%4) * 21), 1, taskDetail);
				if (i == selectedListIndex && _viewstateData->GetNavigationMap()->CurrentSelection()->selectedElement == SELECTABLE_ELEMENT_DYNAMIC)
				{//If current selectable element is SELECTABLE_ELEMENT_DYNAMIC, _selectArrowRight.emplaceLeft of listSelectIndex row
					Viewstate_Element_Text_SelectArrowRight selectArrow(-1, 1);
					emplacedChars += selectArrow.emplaceLeft(target, targetlen, &listRowVE_ScheduledTask);
