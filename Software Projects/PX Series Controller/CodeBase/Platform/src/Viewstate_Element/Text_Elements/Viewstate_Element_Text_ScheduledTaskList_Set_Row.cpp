...
#include "..\..\..\include\Viewstate_Element\Text_Elements\Viewstate_Element_Text_StaticComponentState.h"
...
Viewstate_Element_Text_ScheduledTaskList_Set_Row::Viewstate_Element_Text_ScheduledTaskList_Set_Row(const int start_index, const int imbricationLevel, ScheduledTaskDetail * taskDetail)
	: Viewstate_Element_Text(start_index, imbricationLevel, SELECTABLE_ELEMENT_NOT_SET)
{
	_taskDetail = taskDetail;
}

Viewstate_Element_Text_ScheduledTaskList_Set_Row::~Viewstate_Element_Text_ScheduledTaskList_Set_Row(){}

int Viewstate_Element_Text_ScheduledTaskList_Set_Row::emplace(char * target, const int targetlen)
{
	...
	//emplace task action symbol (turn on/off)
	if (*_taskDetail->GetScheduledTaskAlias() == TaskAlias::TASKALIAS_ENABLE_POWER_RELAY)
	{
		Viewstate_Element_Text_StaticText staticText_taskStatus(_startIndex, 0, Viewstate_String_Generator::onSymbol);
		emplacedCharCount += staticText_taskStatus.emplace(target, targetlen);
	}
	else
	{
		Viewstate_Element_Text_StaticText staticText_taskStatus(_startIndex, 0, Viewstate_String_Generator::offSymbol);
		emplacedCharCount += staticText_taskStatus.emplace(target, targetlen);
	}
	...
	//emplace Enable/Disable symbol
	if (*_taskDetail->GetScheduledTaskEnabledState())
	{
		Viewstate_Element_Text_StaticText staticText_OnOff(_startIndex + 18, 1, Viewstate_String_Generator::enabledSymbol);
		emplacedCharCount += staticText_OnOff.emplace(target, targetlen);
	}
	else
	{
		Viewstate_Element_Text_StaticText staticText_OnOff(_startIndex + 18, 1, Viewstate_String_Generator::disabledSymbol);
		emplacedCharCount += staticText_OnOff.emplace(target, targetlen);
	}
