...
#include "..\..\..\include\Viewstate_Element\Text_Elements\Viewstate_Element_Text_PowerRelayConfigurationList.h"
...
Viewstate_Element_Text_PowerRelayConfigurationList::Viewstate_Element_Text_PowerRelayConfigurationList(const int start_index, const int imbricationLevel, IModelViewstateData * viewstateData, IPowerRelayArray * powerRelays)
...
Viewstate_Element_Text_PowerRelayConfigurationList::~Viewstate_Element_Text_PowerRelayConfigurationList(){}

int Viewstate_Element_Text_PowerRelayConfigurationList::emplace(char * target, const int targetlen)
{
	...
	if (selectedListIndex < 3)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			Viewstate_Element_Text_PowerRelayConfigurationList_Row listRowVE_PowerRelay(_startIndex + 1 + i * 21, 2, i, componentAssociation);
	...
	{		
		...
		for (int i = (((selectedListIndex + 1)/4) * 4) -1; i < (((selectedListIndex + 1) / 4) * 4) + 3; ++i)
		{
			...
			Viewstate_Element_Text_PowerRelayConfigurationList_Row listRowVE_PowerRelay(_startIndex + 1 + (((i + 1) % 4) * 21), 2, i, *association);
